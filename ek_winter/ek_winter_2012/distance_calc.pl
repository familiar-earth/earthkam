...
sub great_circle {
    ...
    $nlon = deg2rad($nlon);
    my $theta = $lon - $nlon;
    my $y = sqrt((cos($nlat) * sin($theta))**2
                 + (cos($lat) * sin($nlat)
                    - sin($lat) * cos($nlat) * cos($theta))**2);
    my $x = cos($lat) * cos($nlat) * cos($theta) + sin($lat) * sin($nlat);
    my $cent_angle = atan2($y, $x);
    return($radius_earth * $cent_angle);
}
