...
II. Description of EarthKAM files
  ...
  II.F ekincl.pl
    ...
    Used by updateNewFields.pl and updateNewFields_single.pl. Mostly a jumble 
    of code that is indecipherable. The only method inside this file is the
    dbupdate routine.
    
  II.G FeatureReader.pm
  
    Has no connection to other files.
  
  II.H frame_incl.pl
  
    Contains routines that calculates the corner points of a lat long box.
    Needed by frame_incl.pl
...
III. Issues that need attention
  ...
  - Check if the corner points are being calculated correctly in farme_incl.pl
